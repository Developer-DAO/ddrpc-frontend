<script lang="ts">
	...
	import { createWalletClient, custom, parseUnits, getContract, createPublicClient, http, type Address } from 'viem';
	...
	const RECIPIENT_ADDRESS = '0x0b2C639c533813f4Aa9D7837CAf62653d097Ff85' as const;
	...
	async function trackConfirmations(publicClient: any, txBlock: bigint) {
		...
		while (attempts < maxAttempts) {
			try {
				...
				if (safeBlock && safeBlock.number && txBlock < safeBlock.number) {
					console.log('Transaction is in safe block, proceeding with backend call');
					return true;
				}
	...
	async function waitForReceipt(publicClient: any, hash: string, maxAttempts = 30) {
	...
	async function processPayment() {
		...
		try {
			...
			try {
				...
				throw new Error(`Please manually switch to ${selectedChain} network in your wallet`);
			}
			...
			if (balance < amount) {
				throw new Error(`Insufficient USDC balance. You need ${total} USDC but have ${Number(balance) / (10 ** chainConfig.decimals)} USDC.`);
			}
			...
			console.log('Transaction mined in block:', initialReceipt.blockNumber);
			
			// Start tracking confirmations until we reach a safe block
			const isSafe = await trackConfirmations(publicClient, initialReceipt.blockNumber);
			
			if (!isSafe) {
				throw new Error('Timeout waiting for safe block. Please try submitting the transaction again.');
			}
	...
	async function fetchTransactions() {
		loading = true;
		try {
			const res = await fetch('http://localhost:3000/api/payments?page=0&per_page=20', {
				method: 'GET',
				credentials: 'include'
			});
	...
	{#if mounted}
		<div class="max-w-4xl mx-auto" transition:fade={{ duration: 300 }}>
			...
			<div class="grid gap-6">
				...
				<div class="bg-neutral-800/50 border border-neutral-700 rounded-lg p-6">
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
						{#each Object.entries(plans) as [id, plan]}
							<div 
								class="border rounded-lg p-4 cursor-pointer transition-all {selectedPlan === id ? 'border-primary-white bg-neutral-700/50' : 'border-neutral-700 hover:border-neutral-600'}"
								on:click={() => selectedPlan = id as keyof typeof plans}
							>
								<h3 class="font-heading text-xl mb-2">{plan.name}</h3>
								<p class="text-2xl font-bold mb-2">${plan.price}/mo</p>
								<p class="text-neutral-400">{plan.requests} requests/month</p>
							</div>
						{/each}
					</div>

					<div class="mb-6">
						<h3 class="font-heading text-xl mb-4">Select Duration</h3>
						<div class="grid grid-cols-4 gap-4">
							{#each durations as duration}
								<div 
									class="border rounded-lg p-4 text-center cursor-pointer transition-all {selectedDuration === duration ? 'border-primary-white bg-neutral-700/50' : 'border-neutral-700 hover:border-neutral-600'}"
									on:click={() => selectedDuration = duration}
								>
									<p class="font-heading">{duration} {duration === 1 ? 'Month' : 'Months'}</p>
								</div>
							{/each}
						</div>
					</div>
					...
					<div class="mb-6">
						<h3 class="font-heading text-xl mb-4">Select Network</h3>
						<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
							{#each Object.entries(chains) as [id, chain]}
								<div 
									class="border rounded-lg p-4 cursor-pointer transition-all {selectedChain === id ? 'border-primary-white bg-neutral-700/50' : 'border-neutral-700 hover:border-neutral-600'}"
									on:click={() => selectedChain = id as keyof typeof chains}
								>
									<p class="font-heading text-lg">{id}</p>
									<p class="text-xs text-neutral-400 mt-1">
										Required Safe Blocks: {chain.safeConfirmations}
										<span class="block">~{chain.timeEstimate} for finality</span>
									</p>
								</div>
							{/each}
						</div>
					...
					<div class="border-t border-neutral-700 pt-4">
						<div class="flex justify-between items-center mb-4">
							...
							<span class="text-2xl font-bold">${calculateTotal()}</span>
						</div>
						<Button
							variant="primary"
						...
						>
							{paymentInProgress ? 'Processing...' : 'Pay with USDC'}
						</Button>
						
						{#if txHash}
							<div class="mt-4 p-3 bg-neutral-900 rounded-lg">
								<p class="text-sm text-neutral-400">Transaction Hash:</p>
								<a 
								...
								{#if waitingForConfirmations}
									<div class="mt-4">
										...
										<p class="text-xs text-neutral-400 mt-2">
											Waiting for transaction to be in a safe block (may take ~{chains[selectedChain].timeEstimate})
										</p>
									</div>
								{/if}
							</div>
				...
				<div class="bg-neutral-800/50 border border-neutral-700 rounded-lg p-6 mb-6">
					<h2 class="font-heading text-2xl mb-4">Manual Transaction Submission</h2>
					<form 
						class="space-y-4"
						on:submit|preventDefault={submitManualTransaction}
					>
						<div>
							<label class="block text-sm text-neutral-400 mb-2">Transaction Hash</label>
							<input
								type="text"
						...
						<div>
							<label class="block text-sm text-neutral-400 mb-2">Network</label>
							<select
						...
						<div class="flex items-center justify-between">
							<p class="text-xs text-neutral-400">
								Directly submit a transaction hash to the backend
							</p>
							<Button
								type="submit"
								variant="secondary"
								...
								<tbody>
									{#each transactions as tx}
										<tr class="border-b border-neutral-700/50">
											...
											<td class="py-3">
												{(Number(tx.amount) / (10 ** tx.decimals)).toFixed(2)} {tx.asset}
											</td>
